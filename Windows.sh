...
# Originally by Kit Knox - Akamai Technologies
#
...
#<UDF name="INSTALL_WINDOWS_VERSION" Label="Windows Version" oneOf="Windows11_Home,Windows11_Education,Windows11_Pro,Windows11_Pro_for_Workstations,Server2019_STD,Server2019_Datacenter,Server2022" default="Server2022"/>
#<UDF name="NIMDA" Label="Admin RDP User" example="nimda" />
#<UDF name="NIMDA_PASSWORD" Label="Admin RDP User Password" example="nimda" />
...
W2K19_ISO_URL='https://chicago-isos.us-ord-1.linodeobjects.com/win2k19.iso'
W11_ISO_URL='https://chicago-isos.us-ord-1.linodeobjects.com/Win11_Eng.iso'
...
if [ $STAGE == 1 ]; then
  ...
  sed -i "s/^#NIMDA=/NIMDA=\"$(echo $NIMDA | sed -e 's/\\/\\\\/g; s/\//\\\//g; s/&/\\\&/g')\"/" /root/StackScript
  sed -i "s/^#NIMDA_PASSWORD=/NIMDA_PASSWORD=\"$(echo $NIMDA_PASSWORD | sed -e 's/\\/\\\\/g; s/\//\\\//g; s/&/\\\&/g')\"/" /root/StackScript
...
stage3() {
  ...
  if ! [ -f windows-amd64.iso ]; then
    if [[ $INSTALL_WINDOWS_VERSION == "Server2019_STD" || $INSTALL_WINDOWS_VERSION == "Server2019_Datacenter" ]]; then
      wget --timeout=90 -q -O windows-amd64.iso $W2K19_ISO_URL
    fi
    if [ $INSTALL_WINDOWS_VERSION == "Server2022" ]; then
      wget --timeout=90 -q -O windows-amd64.iso $W2K22_ISO_URL
    fi
    if [[ $INSTALL_WINDOWS_VERSION == "Windows11_Home" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Education" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Pro" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Pro_for_Workstations" ]]; then
      wget --timeout=90 -q -O windows-amd64.iso $W11_ISO_URL
    fi
  fi
  #sleep 1
  if ! [ -f windows-amd64.iso ]; then
  ...
  if [[ $INSTALL_WINDOWS_VERSION == "Windows11_Home" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Education" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Pro" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Pro_for_Workstations" ]]; then
    PACK_WINDOWS_VERSION=w11
    touch ~/winver.txt
    echo $INSTALL_WINDOWS_VERSION >> ~/winver.txt
    echo $PACK_WINDOWS_VERSION >> ~/winver.txt
  fi
  if [ $INSTALL_WINDOWS_VERSION == "Server2022" ]; then
    PACK_WINDOWS_VERSION=2k22
    touch ~/winver.txt
    echo $INSTALL_WINDOWS_VERSION >> ~/winver.txt
    echo $PACK_WINDOWS_VERSION >> ~/winver.txt
  fi
  if [[ $INSTALL_WINDOWS_VERSION == "Server2019_STD" || $INSTALL_WINDOWS_VERSION == "Server2019_Datacenter" ]]; then
    PACK_WINDOWS_VERSION=2k19
    touch ~/winver.txt
    echo $INSTALL_WINDOWS_VERSION >> ~/winver.txt
    echo $PACK_WINDOWS_VERSION >> ~/winver.txt
  fi
  if [ $INSTALL_WINDOWS_VERSION == "Windows11_Home" ]; then
    INDEX_ORDINAL=1
    echo $INDEX_ORDINAL >> ~/winver.txt
  fi
  if [ $INSTALL_WINDOWS_VERSION == "Server2019_STD" ]; then
    INDEX_ORDINAL=2
    echo $INDEX_ORDINAL >> ~/winver.txt
  fi
  if [[ $INSTALL_WINDOWS_VERSION == "Windows11_Education" || $INSTALL_WINDOWS_VERSION == "Server2019_Datacenter*" ]]; then
    INDEX_ORDINAL=4
    echo $INDEX_ORDINAL >> ~/winver.txt
  fi
  if [ $INSTALL_WINDOWS_VERSION == "Windows11_Pro" ]; then
    INDEX_ORDINAL=6
    echo $INDEX_ORDINAL >> ~/winver.txt
  fi
  if [ $INSTALL_WINDOWS_VERSION == "Windows11_Pro_for_Workstations" ]; then
    INDEX_ORDINAL=10
    echo $INDEX_ORDINAL >> ~/winver.txt
  fi
  if ! [ -f windows_virtio.iso ]; then
    distrobuilder repack-windows windows-amd64.iso windows_virtio.iso --windows-version=$PACK_WINDOWS_VERSION
  fi
  if ! [ -f windows_virtio.iso ]; then
            ...
            <UserAccounts>
                ...
                <LocalAccounts>
                       <LocalAccount wcm:action="add">
                              <Password>
                                     <Value>$NIMDA_PASSWORD</Value>
                                     <PlainText>true</PlainText>
                              </Password>
                              <Description>TF Added Account</Description>
                              <DisplayName>$NIMDA</DisplayName>
...
<unattend xmlns="urn:schemas-microsoft-com:unattend"
    ...
    <cpi:offlineImage cpi:source="wim:c:/wims/install.wim#Windows Server 2019" xmlns:cpi="urn:schemas-microsoft-com:cpi" />
</unattend>
EOF
  fi
  if [[ $INSTALL_WINDOWS_VERSION == "Windows11_Home" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Education" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Pro" ||  $INSTALL_WINDOWS_VERSION == "Windows11_Pro_for_Workstations" ]]; then
    ...
    <settings pass="specialize">
        ...
        <component name="Microsoft-Windows-Deployment" processorArchitecture="amd64" publicKeyToken="31bf3856ad364e35" language="neutral" versionScope="nonSxS" xmlns:wcm="http://schemas.microsoft.com/WMIConfig/2002/State" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
            <RunSynchronous>
                ...
                <RunSynchronousCommand wcm:action="add">
                    <Path>cmd /c net user $NIMDA $NIMDA_PASSWORD /add /passwordchg:no</Path>
                    <Order>5</Order>
                </RunSynchronousCommand>
                <RunSynchronousCommand wcm:action="add">
                    <Path>cmd /c net localgroup Administrators $NIMDA /add</Path>
                    <Order>6</Order>
                </RunSynchronousCommand>
            </RunSynchronous>
        </component>
...
if [ $PACK_WINDOWS_VERSION == "2k22" ]; then
  echo "Installing Windows Server 2022"
fi
if [ $PACK_WINDOWS_VERSION == "2k19" ]; then
  echo "Installing Windows Server 2019"
fi
if [ $PACK_WINDOWS_VERSION == "w11" ]; then
